  ...
  return (
    <main className={styles.main}>
      <form>
        ...
        <input placeholder='Search for Bookmarked Shows' onChange={e => setSearchedItems(e.target.value)} />
      </form>
      { searchedItems === "" ? <div>
        

        <div className={styles.secondComponent}>
          <div className={styles.header}>Bookmarked Movies</div>
          <div className={styles.shows}>
            {bookmark.filter(show => show.category === "Movie").map((datum, index) => <div key={index} className={styles.show}>
        ...
        <div className={styles.secondComponent}>
          <div className={styles.header}>Bookmarked TV Shows</div>
          <div className={styles.shows}>
            {bookmark.filter(show => show.category === "TV Series").map((datum, index) => <div key={index} className={styles.show}>
              <div className={styles.backgroundImage} style={{ backgroundImage: `url(${datum.thumbnail.regular.large})` }}>
                <div className={styles.bookmark} onClick={bookmark.some(val => val.title.includes(datum.title)) ? () => removeItem(datum) : () => bookmarkItems(datum)}>
                    <Image src={bookmark.some(val => val.title.includes(datum.title)) ? bookmark_full : bookmark_empty} width={11} height={14} alt='bookmark' />
                  </div>
              ...
              <div className={styles.boxComponents}>
                <span>{datum.year}</span>
                ●
                <Image src={datum.category === "Movies" ? movies : tv_series} width={12} height={12} alt='cat' />
                <span>{datum.category}</span>
                ●
                <span>{datum.rating}</span>
              </div>
      ...
      <div className={styles.secondComponent}>
          <div className={styles.header}>Found {bookmark.filter(datum => (datum.title).toLowerCase().includes(searchedItems.toLowerCase())).length} results for "{searchedItems}"</div>
          <div className={styles.shows}>
            {bookmark.filter(datum => (datum.title).toLowerCase().includes(searchedItems.toLowerCase())).map((datum, index) => <div key={index} className={styles.show}>
              <div className={styles.backgroundImage} style={{ backgroundImage: `url(${datum.thumbnail.regular.large})` }}>
                <div className={styles.bookmark} onClick={bookmark.some(val => val.title.includes(datum.title)) ? () => removeItem(datum) : () => bookmarkItems(datum)}>
                    <Image src={bookmark.some(val => val.title.includes(datum.title)) ? bookmark_full : bookmark_empty} width={11} height={14} alt='bookmark' />
                  </div>
